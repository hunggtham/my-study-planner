...
export const MovedTasks: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'latest' | 'date_asc' | 'date_desc' | 'moved_count'>('latest');
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    result.sort((a, b) => {
      if (sortBy === 'latest') return new Date(b.updated_at || b.created_at || new Date()).getTime() - new Date(a.updated_at || a.created_at || new Date()).getTime();
      ...
      <header className="page-header" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <select 
            ...
            style={{ background: 'var(--bg-panel)', color: 'var(--text-main)', border: '1px solid var(--border-color)', padding: '0.4rem', borderRadius: '4px' }}
          >
            <option value="all">Tất cả ({tasks.length})</option>
            <option value="pending">Chưa hoàn thành ({tasks.filter(t => t.status !== 'done').length})</option>
            <option value="done">Đã hoàn thành ({tasks.filter(t => t.status === 'done').length})</option>
          </select>

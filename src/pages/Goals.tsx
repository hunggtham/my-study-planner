    ...
    <div className="page-container">
      <header className="page-header" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '1rem' }}>
        ...
        <div className="segmented-control" style={{ maxWidth: '300px' }}>
          <button className={`segmented-btn ${activeTab === 'week' ? 'active' : ''}`} onClick={() => setActiveTab('week')}>Tuần này</button>
          <button className={`segmented-btn ${activeTab === 'month' ? 'active' : ''}`} onClick={() => setActiveTab('month')}>Tháng này</button>
        </div>
      </header>

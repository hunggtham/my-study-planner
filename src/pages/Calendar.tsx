...
export const CalendarView: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list' | 'timeline'>(() => {
    return (localStorage.getItem('study-planner-calendar-view-mode') as any) || 'grid';
  });
  ...
  // Goals modal state
  const [goalsModal, setGoalsModal] = useState<{ type: 'week' | 'month', date: string } | null>(null);
      ...
      <header className="page-header" style={{ alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <div className="segmented-control" style={{ marginTop: '1rem' }}>
            <button className={`segmented-btn ${viewMode === 'grid' ? 'active' : ''}`} onClick={() => setViewMode('grid')}>Grid</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end' }}>
          ...
          <button 
            ...
            onClick={() => setGoalsModal({ type: 'month', date: format(startOfMonth(currentDate), 'yyyy-MM-dd') })}
          >
            Mục tiêu tháng
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          {viewMode === 'grid' && (
            <>
              <div className="calendar-grid">
                ...
                {weeks.map((week, weekIdx) => (
                  <React.Fragment key={`week-${weekIdx}`}>
                    {week.map((day, dayIdx) => {
                      if (!day) return <div key={`empty-${weekIdx}-${dayIdx}`} className="calendar-cell empty"></div>;
                      ...
                      return (
                        <div 
                          ...
                          style={{ cursor: 'pointer', border: selectedDate && isSameDay(day, selectedDate) ? '2px solid var(--primary)' : undefined }}
                        >
                    ...
                    <button 
                      ...
                      onClick={() => {
                        ...
                        if (firstValidDay) setGoalsModal({ type: 'week', date: format(firstValidDay, 'yyyy-MM-dd') });
                      }}
                      title="Mục tiêu tuần"
                    >
                      Mục tiêu
              ...
              {selectedDate && (
                <div className="card" style={{ marginTop: '2rem', animation: 'slideUp 0.3s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <h3 style={{ margin: 0 }}>Chi tiết ngày {format(selectedDate, 'dd/MM/yyyy')}</h3>
                    <button className="secondary-btn" onClick={() => setSelectedDate(null)} style={{ padding: '0.2rem 0.6rem', width: 'auto' }}>Đóng</button>
                  </div>
                      ...
                      <>
                        <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                          ...
                          <button 
                            ...
                            style={{ width: 'auto', margin: 0, padding: '0.4rem 0.8rem' }} 
                            onClick={() => {
                              setEditingTask({ date: format(selectedDate, 'yyyy-MM-dd') });
                              setIsFormOpen(true);
                            }}
                          >
                            + Thêm task
                        ...
                        {dayTasks.length === 0 ? (
                          <div className="text-muted" style={{ padding: '3rem 0', textAlign: 'center', background: 'rgba(0,0,0,0.1)', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {viewMode === 'timeline' && (() => {
            const timedTasks = tasks.filter(t => t.start_time).sort((a, b) => {
